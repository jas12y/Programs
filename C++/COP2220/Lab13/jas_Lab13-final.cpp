...
int lowest(int scores[],int limit)
{
     int low = scores[0];
...
void summary(int limit,int total,string exams[],int scores[])
{
     float average;
     bool lowFound = false;
     int low = lowest(scores,limit);
     
     total = total - low;
     average = (float)total / (limit - 1);
     ...
     cout << "\n\n\n\n\n\n"
          ...
          for (int i = 0; i < limit; i++)
          {
               if (scores[i] == low && lowFound == false)
               {
                   lowFound = true;
                   
                   cout << "\t\t\tScore for " << exams[i] << ":  " << scores[i] << " (Dropped)" << endl;
               }
               else
